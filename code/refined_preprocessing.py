    ...
    with open(structured_nyt_stat_path, encoding='utf8') as stat, \
        ...
        for stat_line in itertools.islice(stat, 2, None, 3):
            ...
            tags = [(TagManager.encode(tag), TagManager.idx(tag))
                    for tag in nyt.readline().split() if TagManager.has_tag(tag)]
            sentences = []
...
def randomly_split_data_in_memory(fraction, x_all, y_all):
    ...
    res = {u'x_train': numpy.zeros(shape=(len(x_all) - eval_data_size, len(x_all[0]))),
           u'y_train': numpy.zeros(shape=(len(y_all) - eval_data_size, len(y_all[0]))),
           u'x_eval': numpy.zeros(shape=(eval_data_size, len(x_all[0]))),
           u'y_eval': numpy.zeros(shape=(eval_data_size, len(y_all[0])))}
    reservoir = []
    idx = 0
    train_idx = 0
    for x in x_all:
